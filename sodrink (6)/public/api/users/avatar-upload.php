...
if (!isset($_FILES['avatar'])) json_error('Fichier manquant (avatar)', 400);
...
// Chemin web (compat. sous-dossier)
$relative = WEB_BASE . '/uploads/avatars/' . $info['filename'];

// Met à jour la base
$repo->update($meId, ['avatar' => $relative]);

// Met à jour la session (pour que le header l’ait immédiatement)
$_SESSION['avatar'] = $relative;

json_success(['avatar' => $relative]);
